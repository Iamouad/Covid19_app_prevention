...
public class ReminderUtilities {
    ...
    public static List<DeviceAppearance> devices = new ArrayList<>();

    public static void addDevice(BluetoothDevice bd, Context context){
        String macAddress = bd.getAddress();
        macAddress = macAddress.substring(0, macAddress.length() - 1);

        if (! Util.containsDev(devices, macAddress)){
            DeviceAppearance device = new DeviceAppearance(System.currentTimeMillis(), macAddress );
            devices.add(device);
            System.out.println("new device added "+ device.getMacAddress());
        }

        else {
            System.out.println("device already exist");
            checkIntervalValidity(macAddress, context);
        }
    }
    ...
    public static void checkIntervalValidity(String macAdd, Context context){
        DeviceAppearance bd = Util.getDevice(devices, macAdd);
        if(bd != null){
            Long currentStamp = System.currentTimeMillis();
            Long startStamp = bd.getFirstAppearance();
            Long diffStamp = currentStamp - startStamp;
            if(diffStamp >= INTERVAL_STAMP){
                //save both devices in data base with one location
                System.out.println("ready to move to database");
                bd.setLastAppearance(bd.getFirstAppearance() + diffStamp);
                System.out.println(bd + " : is going to db");
                MyActualLocation actualLocation = new MyActualLocation(context, bd);
                actualLocation.getLastLocation();
                devices.remove(bd);
            }
            else {
                System.out.println("current "+ currentStamp);
                System.out.println("start "+ startStamp);
                System.out.println("diff "+ diffStamp);
                System.out.println("Interval: " + INTERVAL_STAMP);
                System.out.println("not ready to move to database");
            }
        }
